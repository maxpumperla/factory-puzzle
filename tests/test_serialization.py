...
def test_serde():
    ...
    assert json.dumps(node_to_dict(node)) == \
           '{"name": "foo", "neighbours": {"right": [1, 0]}, ' \
           '"is_rail": false, "has_shuttle": true, "coordinates": [0, 0]}'

    table = Table(node=node, core=None, name="bar")
    assert json.dumps(table_to_dict(table)) == \
           '{"name": "bar", "coordinates": [0, 0]}'

    phases = {Phase.A: node}
    core = Core(table=table, cycle=phases, name="baz")
    assert json.dumps(core_to_dict(core)) == \
           '{"name": "baz", "coordinates": [0, 0], "current_phase": "A", "current_target_coordinates": [0, 0]}'
